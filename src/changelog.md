<!--
  User-facing changelog, newest first. One release per update: group everything that
  ships together (e.g. the same day) into a single version rather than bumping per change.
  Each release is a "## version — date" header followed by "- " bullets, one per line.
  Plain text only — no markdown styling is rendered. The top release shows the "New"
...
- There’s a Share button on your results now.
